...
def find_nilai_tukar(text, marker_pairs,kuartal):
    ...
    lebih_rendah = f"USD{nominal_matches[0]} juta" if len(nominal_matches) > 0 else "Tidak ditemukan"
    lebih_tinggi = f"USD{nominal_matches[1]} juta" if len(nominal_matches) > 1 else "Tidak ditemukan"
